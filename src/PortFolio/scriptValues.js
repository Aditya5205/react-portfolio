...
const useScript = (page) => {
  ...
  const shape = textureLoader.load("../assets/particleShape/1.png");
  ...
  function generateGalaxy() {
    ...
    if (pts.length !== 0) {
      points.position.set(pts[0], pts[1], pts[2]);
    }
    scene.add(points);
  }
  ...
  // camera.position.z = 3;
  if (pts.length !== 0) {
    camera.position.set(pts[3], pts[4], pts[5]);
  }
  scene.add(camera);
